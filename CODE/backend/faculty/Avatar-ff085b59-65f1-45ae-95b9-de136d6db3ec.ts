...
//update the faculty
router.put('/faculty/:facultyId', uploads.fields([{ name: 'Avatar', maxCount: 1 }]), async (req: any, res: Response, next: NextFunction) => {
    const facultyId = req.params.facultyId;
    try {
        ...
        const  updateimage = req.files['Avatar'] ? req.files['Avatar'][0].filename : existingfaculty.Avatar
...
router.put('/student/:studentId', upload.fields([{ name: 'Avathar', maxCount: 1 }]), async (req: any, res: any, next: NextFunction) => {
    const studentId = req.params.studentId;
    try {
...
const uploadss = multer({ storage: store });

router.put('/updateevent/:eventId', uploadss.array('pic', 3) , async (req: any, res: any, next: NextFunction) => {
