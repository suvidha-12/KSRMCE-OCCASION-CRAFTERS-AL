...
try {
  await AxiosAPI.put(`faculty/faculty/${facultyId}`, myData).then((response)=>{
 ...
 toast.error(error.response.data.message)
  ...
  const getFac = async () => {
    await AxiosAPI.get(`/faculty/faculty/${facultyId}`).then((response) => {
      console.log(response, "all students");
      for(let [key, value] of Object.entries(response.data.faculty
        )){
       setValue(key, value);
      }
    });
  ...
  return (
    <div>
        ...
        {isLoading?<i className="fa-solid fa-spinner fa-spin text-white fs-3 spinners"></i>:""}
            ...
            <h3 className='register-title'> Update Profile</h3>
