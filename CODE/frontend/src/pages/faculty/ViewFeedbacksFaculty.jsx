...
const ViewFeedbacksFaculty = () => {
  const [queries, setQueries] = useState();
  const faculty = JSON.parse(localStorage.getItem("faculty"));

  const getQueries = async () => {
    try {
      await AxiosAPI.get(`faculty/feedbacks/${faculty._id}`).then((respone) => {
  ...
  const getAllEvents=async()=>{
    try {
      await AxiosAPI.get(`/faculty/event/${faculty._id}`).then((resp)=>{
              ...
              {queries &&
                queries.map((item) => (
                  <li className="p-3 " key={item._id}>
                    ...
                    <p className="" style={{marginLeft:"50px"}}> Event Name:
  {
    // Attempt to find the event in the events array that matches item.event
    (() => {
      const event = events.find(eventItem => eventItem._id === item.event);
      // If an event is found, return its title; otherwise, return the default message
      return event ? event.title : "This Event expired or deleted from database";
    })()
  }
</p>
                    <p className=" ms-lg-5 ">{item.feedback}</p>
                  </li>
                ))}
              {/* <li className="p-3 ">
...
export default ViewFeedbacksFaculty;
